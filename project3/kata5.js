...
function mean(town, strng) {
   const townsRainfall = strng.split(/\r?\n/);
   let townIndex = -1;
   for(let i = 0; i <= townsRainfall.length; i++){
     if(townsRainfall[i].includes(town)) {
       townIndex = i;
       break;
     }
   };
  const plainRainfall = townsRainfall[townIndex].match(/[-]{0,1}[\d]*[\.]{0,1}[\d]+/g);
  return  math.mean(plainRainfall);
}
...
function variance(town, strng) {
  ...
  const plainRainfall = townsRainfall[townIndex].match(/[-]{0,1}[\d]*[\.]{0,1}[\d]+/g);
  console.log(plainRainfall)
  return math.var(plainRainfall);
}
